...
class DSSYOLOController:
    ...
    def handle_key_control(self, key):
        ...
        if not self.edit_mode:
            ...
            print(
                f"[CONTROL] T:{self.drive_state['throttle']:.2f} S:{self.drive_state['steer']:.2f} B:{self.drive_state['brake']:.2f}")
    ...
    def draw_anchor_lines_and_center(self, image, mask):
        ...
        for i, y in enumerate(y_positions):
            ...
            if len(lane_pixels) > 0:
                ...
                self.lane_detector.draw_anchor_line_excluding_car(result_image, y, width, car_region, line_color,
                                                                  line_thickness)
    ...
    def compute_stanley_control(self, center_points, vehicle_position_x, heading, k=0.5):
        ...
        # 디버깅 출력 추가
        print(
            f"[Stanley Debug] dx: {dx:.2f}, dy: {dy:.2f}, path_angle(deg): {np.degrees(path_angle):.2f}, heading(deg): {np.degrees(heading):.2f}, heading_error(deg): {np.degrees(heading_error):.2f}")
        print(
            f"[Stanley] dx: {dx:.2f}, dy: {dy:.2f}, cross_track_error: {cross_track_error:.4f}, heading_error: {heading_error:.4f}, steer(deg): {steer_deg:.4f}")
    ...
    def on_camera_data(self, image: dss_pb2.DSSImage):
        ...
        try:
            ...
            if self.lane_detector.model is not None:
                results = self.lane_detector.model(self.rgb_image, conf=self.lane_detector.conf_threshold,
                                                   verbose=False)
                if results and results[0].masks:
                    ...
                    if self.show_anchor_lines:
                        lane_detected_image, center_points = self.draw_anchor_lines_and_center(overlay_image,
                                                                                               combined_mask)
...
def main():
    ...
    EXPERIMENT_NAME = "DSS_experiment_640x480_1"

    MODEL_PATH = os.path.join(BASE_PATH, "DSS_AI_training_640x480", EXPERIMENT_NAME, "weights", "best.pt")
